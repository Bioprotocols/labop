...
reagent_rack = paml.ContainerSpec('reagent_rack', name='Tube rack for reagents', queryString='cont:Opentrons24TubeRackwithEppendorf1.5mLSafe-LockSnapcap', prefixMap=PREFIX_MAP)
...
primer_plate = paml.ContainerSpec('primer_plate', name='primers in 96-well plate', queryString='cont:Corning96WellPlate360uLFlat', prefixMap=PREFIX_MAP)
...
polymerase = paml.ContainerSpec('polymerase', name='DNA Polymerase', queryString='cont:StockReagent', prefixMap=PREFIX_MAP)
...
# Set up water
load_water = protocol.primitive_step('LoadContainerInRack', slots=rack.output_pin('slots'), container=paml.ContainerSpec('water', name='tube for water', queryString='cont:MicrofugeTube', prefixMap=PREFIX_MAP) , coordinates='B1')
...
for template, coordinate in zip(templates, rack_coordinates):
    ...
    doc.add(c_template)
    template_container = paml.ContainerSpec(sanitize_display_id(template), name='container of ' + template, queryString='cont:MicrofugeTube', prefixMap=PREFIX_MAP)
...
# Set up PCR machine
pcr_plate = paml.ContainerSpec('pcr_plate', name='PCR plate', queryString='cont:Biorad96WellPCRPlate', prefixMap=PREFIX_MAP)

...
def define_pH_adjustment_protocol_primitives(document: sbol3.Document, library: str):
    ...
    calculate_naoh_addition = declare_primitive(
        ...
        inputs=[
            ...
            {
                "name": "temperature",
                "type": "http://www.ontology-of-units-of-measure.org/resource/om-2/degreeCelsius",
            },
...
def wrap_with_error_message(protocol, library, primitive, **kwargs):
    ...
    wrapped_primitive.compute_output = wrapped_primitive_compute_output
    ...
    error_message_invocation = protocol.execute_primitive(
        error_message_primitive,
...
def get_ph_calibration_protocol_inputs(
    protocol: labop.Protocol,
) -> Tuple[uml.ActivityParameterNode]:
...
def define_pH_calibration_protocol_primitives(document: sbol3.Document, library: str):
    ...
    mix_primitive = declare_primitive(
        ...
        "Mix",
        inputs=[
            {"name": "samples", "type": labop.SampleArray},
    ...
    clean_electrode_primitive.compute_output = clean_electrode_primitive_compute_output
...
def define_setup_protocol_materials(document):
    ...
    return (h3po4, ddh2o, naoh)


def get_setup_protocol_inputs(
    protocol: labop.Protocol,
) -> Tuple[uml.ActivityParameterNode]:
    """Declare and return the inputs for the pH calibration protocol.

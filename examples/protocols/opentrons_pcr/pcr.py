...
def run(pcr_plan: str, primers: str, templates: str, output: str):
    ...
    ############################3
    # load pcr setup
    primer_layout = load_primer_layout(primers, doc)
    template_layout = load_templates(templates, doc)
    pcr_plan = load_pcr_plan(pcr_plan, doc)
    ...
    load = protocol.primitive_step(
        "LoadRackOnInstrument", rack=spec_tiprack, coordinates="4"
    )
    ...
    load = protocol.primitive_step(
        "LoadRackOnInstrument", rack=primer_plate, coordinates="1"
    )
    primer_plate = protocol.primitive_step("EmptyContainer", specification=primer_plate)

    # Set up templates plate
    template_plate = labop.ContainerSpec(
        "template_plate",
        name="templates in 96-well plate",
        queryString="cont:Corning96WellPlate360uLFlat",
        prefixMap=PREFIX_MAP,
    )
    load = protocol.primitive_step(
        "LoadRackOnInstrument", rack=template_plate, coordinates="3"
    )
    template_plate = protocol.primitive_step(
        "EmptyContainer", specification=template_plate
    )
    ...
    for target_well, reaction in pcr_plan.items():
        ...
        [f_primer_coordinates] = [
            c for c, p in primer_layout.items() if doc.find(p).name == f_primer
        ]
        r_primer = reaction["Reverse"]
        [r_primer_coordinates] = [
            c for c, p in primer_layout.items() if doc.find(p).name == r_primer
        ]
        template = reaction["Template"]
        [template_coordinates] = [
            c for c, t in template_layout.items() if doc.find(t).name == template
        ]
        ...
        transfer = protocol.primitive_step(
            ...
            amount=sbol3.Measure(9, tyto.OM.microliter),
        )
        ...
        # Transfer template
        template_sample = protocol.primitive_step(
            "PlateCoordinates",
            source=template_plate.output_pin("samples"),
            coordinates=template_coordinates,
        )
        transfer = protocol.primitive_step(
            "Transfer",
            source=template_sample.output_pin("samples"),
            destination=target_sample.output_pin("samples"),
            amount=sbol3.Measure(1, tyto.OM.microliter),
        )
        ...
        transfer = protocol.primitive_step(
            ...
            destination=target_sample.output_pin("samples"),
            amount=sbol3.Measure(12.5, tyto.OM.microliter),
        )
        transfer.name = "Add 2X master mix"
    ...
    ee = ExecutionEngine(specializations=[OT2Specialization(output)], failsafe=False)
    parameter_values = []
...
def load_primer_layout(fname: str, doc: sbol3.Document):
    ...
    with open(fname, "r") as f:
        ...
        for row in reader:
            primer = row["Sequence Name"]
            coordinate = row["Well Position"]
...
def load_templates(fname: str, doc: sbol3.Document):
    """Load templates from a 96 well plate into a dict"""
    contents = {}
    with open(fname, "r") as f:
        reader = csv.DictReader(f)
        templates = set()
        for row in reader:
            ...
            template = sbol3.Component(
                "Component/" + sanitize_display_id(template),
                name=template,
                types=sbol3.SBO_DNA,
                roles=[tyto.SO.plasmid],
            )
            if not doc.find(template.identity):
                doc.add(template)
            contents[coordinate] = template.identity
...
if __name__ == "__main__":
    ...
    DEFAULT_PLAN = os.path.join(DEFAULT_DIR, "data", "pcr_plan.csv")
    DEFAULT_PRIMERS = os.path.join(DEFAULT_DIR, "data", "primer_layout.csv")
    DEFAULT_TEMPLATES = os.path.join(DEFAULT_DIR, "data", "template_layout.csv")
    DEFAULT_OUTPUT = os.path.join(DEFAULT_DIR, "artifacts", "ot2_pcr_labop.py")
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--plan",
        default=DEFAULT_PLAN,
        help=f"Filename of the PCR plan in a CSV file.",
    )
    parser.add_argument(
        "--primers",
        default=DEFAULT_PRIMERS,
        help=f"Filename of the primers CSV file",
    )
    parser.add_argument(
        "--templates",
        default=DEFAULT_TEMPLATES,
        help=f"Filename for a CSV file containing PCR templates.",
    )
    parser.add_argument(
        "--output",
    ...
    run(args.plan, args.primers, args.templates, args.output)

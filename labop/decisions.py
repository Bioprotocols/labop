...
def protocol_make_decision_node(
    ...
    primary_incoming_flow = (
        uml.ControlFlow(source=primary_incoming_node)
        if isinstance(primary_incoming_node, uml.ControlNode)
        else uml.ObjectFlow(source=primary_incoming_node)
    )
    ...
    decision = uml.DecisionNode(
        decision_input=decision_input_behavior, decision_input_flow=decision_input_flow
    )
    ...
    if decision_input:
        ...
        decision_input_to_decision_flow = uml.ObjectFlow(
...
labop.Protocol.make_decision_node = (
    protocol_make_decision_node  # Add to class via monkey patch
)
...
def decision_node_add_decision_output(self, protocol, guard, target):
    ...
    outgoing_edge = (
        uml.ObjectFlow(**kwargs)
        if isinstance(self.get_primary_incoming_flow(protocol).source, uml.ObjectNode)
        else uml.ControlFlow(**kwargs)
    )
    protocol.edges.append(outgoing_edge)


uml.DecisionNode.add_decision_output = (
    decision_node_add_decision_output  # Add to class via monkey patch
)
...
def decision_node_get_primary_incoming_flow(self, protocol):
    try:
        primary_incoming_flow = next(
            ...
            and not (
                isinstance(e.source.lookup(), uml.OutputPin)
...
uml.DecisionNode.get_primary_incoming_flow = (
    decision_node_get_primary_incoming_flow  # Add to class via monkey patch
)
...
uml.DecisionNode.get_decision_input_node = (
    decision_node_get_decision_input_node  # Add to class via monkey patch
)
...
uml.ForkNode.get_decision_input_node = (
    fork_node_get_decision_input_node  # Add to class via monkey patch
)
...
uml.OutputPin.get_decision_input_node = (
    output_pin_get_decision_input_node  # Add to class via monkey patch
)

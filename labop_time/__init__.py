...
def timePointExpression(element: uml.Behavior, interval: uml.TimeInterval, first=True):
    ...
    return uml.TimeConstraint(
        constrained_elements=[_referencedOrderedPropertyValue(0, element)],
...
def constrainDuation(element: uml.Behavior, interval, units=tyto.OM.second):
    return durationExpression(
        element, _getUMLInterval(interval, uml.DurationInterval, units=units)
    )
...
def binaryDuration(
    ...
    ordered_elements = [
        _referencedOrderedPropertyValue(0, element1),
    ...
    return uml.DurationConstraint(
        ...
        specification=interval,
        firstEvent=[
            _orderedPropertyValue(0, uml.LiteralBoolean(value=first1)),

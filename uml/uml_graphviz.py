...
def activity_to_dot(self, legend=False, ready=[], done=[]):
    ...
    def _type_attrs(object: ActivityNode, ready, done) -> Dict[str,str]:
        ...
        if isinstance(object, InitialNode):
            node_attrs = {'label': '', 'shape': 'circle', 'style': 'filled', 'fillcolor': color }
        elif isinstance(object, FinalNode):
            node_attrs = {'label': '', 'shape': 'doublecircle', 'style': 'filled', 'fillcolor': color}
        elif isinstance(object, ForkNode) or isinstance(object, JoinNode):
            node_attrs = {'label': '', 'shape': 'rectangle', 'height': '0.02', 'style': 'filled', 'fillcolor': color}
            ...
            node_attrs = {'label': label, 'shape': 'rectangle', 'peripheries': '2', 'fillcolor': color}
            ...
            if isinstance(object, CallBehaviorAction): # render as an HMTL table with pins above/below call
                ...
                raise ValueError(f'Do not know what GraphViz label to use for {object}')
            node_attrs = {'label': label, 'shape': shape, 'style': 'rounded', 'fillcolor': color}
        else:
    ...
    try:
        ...
        for edge in self.edges:
            ...
            if isinstance(source, DecisionNode) and hasattr(edge, "guard"):
                label = edge.guard.value if edge.guard and not isinstance(edge.guard, LiteralNull) and edge.guard.value is not None else "None"
                label = "Else" if label == 'http://bioprotocols.org/uml#else' else str(label)
